...
internal class RadioService : Service() {
    ...
    private val handler =
        Handler(handlerThread.looper) { msg ->
            ...
            when (msg.what) {
                ...
                // ONE-TAP OVERLAY GRANT: a signed client asks the helper to grant it
                // "draw over other apps" (SYSTEM_ALERT_WINDOW) via the shell-UID
                // self-ADB channel. The client's OWN package rides in the message
                // Bundle (getCallingUid is unreliable in a Messenger handler). BIND_RADIO
                // is a signature perm, so only same-key apps reach here. Reply arg1 = 1
                // if the op verified 'allow' afterward.
                MSG_GRANT_OVERLAY -> {
                    val pkg = msg.data?.getString(KEY_OVERLAY_PKG)
                    replyResult(msg, pkg != null && RadioToggler.grantOverlay(this, pkg))
                }
            }
            true
        }
    ...
    companion object {
        ...
        /**
         * ONE-TAP OVERLAY GRANT — a signed client asks the helper to grant it
         * SYSTEM_ALERT_WINDOW ("draw over other apps") via the shell-UID self-ADB
         * channel. The target package is passed in the message data Bundle under
         * [KEY_OVERLAY_PKG]. Reply arg1 = 1 if verified granted. The appop persists
         * across reboot, so this is a one-shot with no per-boot step.
         */
        const val MSG_GRANT_OVERLAY = 7
        const val KEY_OVERLAY_PKG = "pkg"
    }
}
