...
internal object RadioToggler {
    ...
    /**
     * ONE-TAP OVERLAY GRANT — grant "draw over other apps" (SYSTEM_ALERT_WINDOW) to
     * [pkg] silently via the shell-UID self-ADB channel (the same one-time-paired
     * Wireless-Debugging connection that flips Wi-Fi). Unlike Wi-Fi there is no
     * "direct" or panel rung for an appop — only shell-UID (self-ADB) or root — so
     * this is self-ADB only (a Shizuku rung could be added later for users already
     * running it). The appop PERSISTS across reboot ⇒ one-shot, no per-boot step.
     * Blocks (socket/mDNS I/O) — [RadioService] calls it on its HandlerThread.
     * @return true only if verified 'allow' afterward.
     */
    fun grantOverlay(
        context: Context,
        pkg: String,
    ): Boolean {
        val ok = AdbWifiManager.grantOverlay(context, pkg)
        Log.i(TAG, "grantOverlay($pkg) via self-ADB = ${if (ok) "OK" else "FAILED"}")
        return ok
    }
}
