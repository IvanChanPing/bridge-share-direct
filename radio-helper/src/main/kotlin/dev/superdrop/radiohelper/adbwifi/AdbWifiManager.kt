...
internal class AdbWifiManager private constructor(
    ...
    internal companion object {
        ...
        /**
         * Grant "draw over other apps" (SYSTEM_ALERT_WINDOW appop) to [targetPkg]
         * over the self-ADB shell (shell UID), then VERIFY it actually holds — the
         * same set-then-verify discipline as [selfGrantWriteSecureSettings], because
         * [runShell] returns non-null even when `appops set` prints an error. The
         * appop is stored in /data/system/appops.xml and PERSISTS across reboot, so
         * this is a ONE-SHOT grant with no per-boot step. Blocking — off the main
         * thread. @return true ONLY when `appops get` reports the op `allow` after.
         */
        fun grantOverlay(
            context: Context,
            targetPkg: String,
        ): Boolean {
            val setOut = runShell(context, "appops set $targetPkg SYSTEM_ALERT_WINDOW allow")
            val getOut = runShell(context, "appops get $targetPkg SYSTEM_ALERT_WINDOW")
            lastGrantOutput = "set='${setOut?.trim()}' get='${getOut?.trim()}'"
            val held =
                getOut != null &&
                    getOut.contains("SYSTEM_ALERT_WINDOW", ignoreCase = true) &&
                    getOut.contains("allow", ignoreCase = true)
            when {
                setOut == null -> Log.w(TAG, "grantOverlay: shell never connected ($lastError)")
                !held -> Log.w(TAG, "grantOverlay: ran but op != allow. $lastGrantOutput")
                else -> Log.i(TAG, "grantOverlay: SYSTEM_ALERT_WINDOW granted+verified for $targetPkg")
            }
            return held
        }
    }
